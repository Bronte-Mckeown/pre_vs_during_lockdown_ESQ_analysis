"This script runs Linear Mixed Models with Sample, Social environment &
...
# exclude anyone aged 28-54
# 4715 observations left
exclude <- c(28:54)
df1 <- df1 %>%
  filter(!(age %in% exclude))
...
# Globally set lmertest.limit to 4870 to calculate df for emmeans plots
# also set to sattherthwaite
emm_options(lmerTest.limit = 4870, lmer.df = "satterthwaite")

########################## Saving results ######################################
#set file name for lmer text output
fp = "lmm_pca_thoughts_by_sample_social_agegroup_centered_age_agerange_limit.txt"
...
# NOTE: to match tables in supplementary materials, factor levels need re-ordering
myfile2 <- file.path(results_dir,"thoughts_sample_social_agegroup_limit_agerange_summary_tables")
...
myfile3 <- file.path(results_dir,"thoughts_sample_social_agegroup_limit_agerange_anova_tables")
...
ggsave(
  "thoughts_sample_social_agegroup_limit_agerange.tiff",
  all_plots, units = "cm",
  width = 14,
  height = 15,
  dpi = 1000, 
...
ggsave(
  "thoughts_sample_social_agegroup_limit_agerange.pdf",
  all_plots, units = "cm",
  width = 11.4,
  height = 15,
  dpi = 1000, 
...
fp2 <- "lmm_pca_thoughts_by_sample_social_agegroup_centered_age_agerange_limit_posthoc.txt"
...
# shows that the effect of social environment is greater in lockdown sample
...
# Indicates that direction of social environment changes for young PS in lockdown
...
# use emmean4 to run contrast of contrasts (already set in model loop)
...
fac4.contrast.of.contrasts <- contrast(emmean4, method = list("(before.alone.young-before.interacting.young) - (during.alone.young - during.interacting.young) " 
...
cat("\n\n\n", file = fp2, append = TRUE)
capture.output(fac4.contrast.of.contrasts,file = fp2, append = TRUE)
cat("\n\n\n", file = fp2, append = TRUE)
